# 历史源码：tests/test_excel_structured_loader.py

来源：个人分支提交 `8aba26b`。仅供学习，保留原始内容；导入、环境及接口属于旧版，不是当前运行入口。当前测试见 `tests/test_excel_structured_loader.py`；当前评测见 `backend/evaluation/`。
