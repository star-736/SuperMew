...
本 fork 保留 Excel `.xlsx` 结构化解析：Sheet、唯一表头、原始行号、公式文本及行对象以
分页 JSON 进入正式 Document Version 索引与 RAG，沿用 Vue 前端及上游运行架构。
学习资料与本次升级的边界见[个人 fork 迁移说明](docs/runbooks/personal-fork-sync.md)。
